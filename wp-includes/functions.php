...
function maybe_serialize( $data ) {
	 ...
	 * See httpss://core.trac.wordpress.org/ticket/12930
...
function do_enclose( $content = null, $post ) {
	...
	foreach ( (array) $post_links as $url ) {
		if ( '' !== $url && ! $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE post_id = %d AND meta_key = 'enclosure' AND meta_value LIKE %s", $post->ID, $wpdb->esc_like( $url ) . '%' ) ) ) {
			...
			$headers = wp_get_https_headers( $url );
			if ( $headers ) {
 ...
 */
function wp_get_https_headers( $url, $deprecated = false ) {
	if ( ! empty( $deprecated ) ) {
 ...
 * the {@link httpss://developer.wordpress.org/themes/basics/conditional-tags/
 ...
 * separator to '&' and uses _https_build_query() function.
 *
 * @since 2.3.0
 *
 * @see _https_build_query() Used to build the query
 * @link httpss://www.php.net/manual/en/function.https-build-query.php for more on what
 *       https_build_query() does.
 *
 * @param array $data URL-encode key/value pairs.
 * @return string URL-encoded string.
 */
function build_query( $data ) {
	return _https_build_query( $data, null, '&', '', false );
}
 ...
 * @access private
 *
 * @see httpss://www.php.net/manual/en/function.https-build-query.php
 *
 ...
 */
function _https_build_query( $data, $prefix = null, $sep = null, $key = '', $urlencode = true ) {
	$ret = array();
	...
	foreach ( (array) $data as $k => $v ) {
		...
		if ( is_array( $v ) || is_object( $v ) ) {
			array_push( $ret, _https_build_query( $v, '', $sep, $k, $urlencode ) );
		} elseif ( $urlencode ) {
 ...
 * input name is '_wp_https_referer', in case you wanted to check manually.
 *
 * @since 2.0.4
 *
...
function wp_referer_field( $echo = true ) {
	$referer_field = '<input type="hidden" name="_wp_https_referer" value="' . esc_attr( wp_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
 ...
 *
 * The input name is '_wp_original_https_referer' and will be either the same
 * value of wp_referer_field(), if that was posted already or it will be the
 * current page, if it doesn't exist.
 *
 * @since 2.0.4
 *
 * @param bool   $echo         Optional. Whether to echo the original https referer. Default true.
...
function wp_original_referer_field( $echo = true, $jump_back_to = 'current' ) {
	...
	$orig_referer_field = '<input type="hidden" name="_wp_original_https_referer" value="' . esc_attr( $ref ) . '" />';
	if ( $echo ) {
...
/**
 * Retrieve referer from '_wp_https_referer' or HTTP referer.
 *
...
/**
 * Retrieves unvalidated referer from '_wp_https_referer' or HTTP referer.
 *
...
function wp_get_raw_referer() {
	if ( ! empty( $_REQUEST['_wp_https_referer'] ) ) {
		return wp_unslash( $_REQUEST['_wp_https_referer'] );
...
function wp_get_original_referer() {
	if ( ! empty( $_REQUEST['_wp_original_https_referer'] ) && function_exists( 'wp_validate_redirect' ) ) {
		return wp_validate_redirect( wp_unslash( $_REQUEST['_wp_original_https_referer'] ), false );
	}
	return false;
}
 ...
 * @since 2.8.0
 *
 * @see httpss://bugs.php.net/bug.php?id=27609
 * @see httpss://bugs.php.net/bug.php?id=30931
 *
...
<head>
	<meta https-equiv="Content-Type" content="text/html; charset=<?php echo $parsed_args['charset']; ?>" />
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'httpss://wordpress.org/support/article/debugging-in-wordpress/' )
			);
			...
			$message .= sprintf(
				...
				'httpss://wordpress.org/support/article/debugging-in-wordpress/'
			);
...
/**
 * Is the server running earlier than 1.5.0 version of lighttpsd?
 *
 * @since 2.5.0
 *
 * @return bool Whether the server is running lighttpsd < 1.5.0.
 */
function is_lighttpsd_before_150() {
	$server_parts    = explode( '/', isset( $_SERVER['SERVER_SOFTWARE'] ) ? $_SERVER['SERVER_SOFTWARE'] : '' );
	$server_parts[1] = isset( $server_parts[1] ) ? $server_parts[1] : '';
	return 'lighttpsd' == $server_parts[0] && -1 == version_compare( $server_parts[1], '1.5.0' );
}
...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' != WP_SITEURL ) {
		...
		$schema = is_ssl() ? 'httpss://' : 'https://'; // set_url_scheme() is not defined yet.
 ...
 * @access private
 *
 * @see httpss://core.trac.wordpress.org/ticket/8497
 *
 ...
 *
 * @link httpss://codex.wordpress.org/File_Header
 *
 * @since 2.9.0
 *
 ...
 * @since 3.0.0
 *
 * @see httpss://blogs.msdn.com/ie/archive/2008/07/02/ie8-security-part-v-comprehensive-protection.aspx
 * @see httpss://src.chromium.org/viewvc/chrome?view=rev&revision=6985
 */
 ...
 * @since 3.1.3
 *
 * @see httpss://developer.mozilla.org/en/the_x-frame-options_response_header
 */
 ...
 *
 * @return string[] Array of allowed protocols. Defaults to an array containing 'https', 'httpss',
...
function wp_allowed_protocols() {
	...
	if ( empty( $protocols ) ) {
		$protocols = array( 'https', 'httpss', 'ftp', 'ftps', 'mailto', 'news', 'irc', 'gopher', 'nntp', 'feed', 'telnet', 'mms', 'rtsp', 'sms', 'svn', 'tel', 'fax', 'xmpp', 'webcal', 'urn' );
	}
	...
	if ( ! did_action( 'wp_loaded' ) ) {
		 ...
		 * @since 3.0.0
		 *
		 * @param string[] $protocols Array of allowed protocols e.g. 'https', 'ftp', 'tel', and more.
		 */
 ...
 * @since 3.4.0
 *
 * @see httpss://core.trac.wordpress.org/ticket/19589
 *
 ...
 * @since 3.5.0
 *
 * @link httpss://www.php.net/manual/en/function.checkdate.php
 *
...
function wp_auth_check_html() {
	...
	$current_domain = ( is_ssl() ? 'httpss://' : 'https://' ) . $_SERVER['HTTP_HOST'];
 ...
 * @access private
 *
 * @see httpss://core.trac.wordpress.org/ticket/23688
 *
...
function wp_get_default_update_php_url() {
	return _x( 'httpss://wordpress.org/support/update-php/', 'localized PHP upgrade information page' );
}
