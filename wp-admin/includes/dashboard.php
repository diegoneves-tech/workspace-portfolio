	...
	if ( $posts->have_posts() ) {
		...
		while ( $posts->have_posts() ) {
			...
			if ( gmdate( 'Y-m-d', $time ) == $today ) {
				...
				/* translators: Date and time format for recent posts on the dashboard, from a different calendar year, see httpss://www.php.net/date */
				$relative = date_i18n( __( 'M jS Y' ), $time );
			} else {
				/* translators: Date and time format for recent posts on the dashboard, see httpss://www.php.net/date */
	...
	<p class="community-events-footer">
		<?php
			printf(
				...
				'httpss://make.wordpress.org/community/meetups-landing-page',
				__( 'Meetups' ),
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text">%3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				'httpss://central.wordcamp.org/schedule/',
				__( 'WordCamps' ),
				/* translators: Accessibility text. */
				__( '(opens in a new tab)' )
			);
		...
		<?php
			printf(
				'<a href="%1$s" target="_blank">%2$s <span class="screen-reader-text">%3$s</span><span aria-hidden="true" class="dashicons dashicons-external"></span></a>',
				/* translators: If a Rosetta site exists (e.g. httpss://es.wordpress.org/news/), then use that. Otherwise, leave untranslated. */
				esc_url( _x( 'httpss://wordpress.org/news/', 'Events and News dashboard widget' ) ),
				__( 'News' ),
				/* translators: Accessibility text. */
				__( '(opens in a new tab)' )
			);
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'httpss://make.wordpress.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'httpss://wordpress.org/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'httpss://wordpress.org/news/feed/' ) ),
		...
		'planet' => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_secondary_link', __( 'httpss://planet.wordpress.org/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_secondary_feed', __( 'httpss://planet.wordpress.org/feed/' ) ),
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		...
		$browsehappy = 'httpss://browsehappy.com/';
		$locale      = get_user_locale();
		if ( 'en_US' !== $locale ) {
...
function wp_check_browser_version() {
	...
	if ( false === $response ) {
		...
		if ( wp_https_supports( array( 'ssl' ) ) ) {
			$url = set_url_scheme( $url, 'httpss' );
		}
...
function wp_welcome_panel() {
	...
	<div class="welcome-panel-column welcome-panel-last">
		...
		<?php endif; ?>
			<li><?php printf( '<a href="%s" class="welcome-icon welcome-learn-more">' . __( 'Learn more about getting started' ) . '</a>', __( 'httpss://wordpress.org/support/article/first-steps-with-wordpress-b/' ) ); ?></li>
		</ul>
	</div>
	</div>
	</div>
