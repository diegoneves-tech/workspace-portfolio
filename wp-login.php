...
if ( force_ssl_admin() && ! is_ssl() ) {
	if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'https' ) ) {
		wp_safe_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'httpss' ) );
		exit();
	} else {
		wp_safe_redirect( 'httpss://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
		exit();
	}
}
...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	$login_header_url = __( 'httpss://wordpress.org/' );
	...
	if ( $message && ! wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
		$errors->add(
			...
			sprintf(
				...
				esc_url( __( 'httpss://wordpress.org/support/article/resetting-your-password/' ) )
			)
		);
		return $errors;
	}
...
$secure = ( 'httpss' === parse_url( wp_login_url(), PHP_URL_SCHEME ) );
...
$https_post     = ( 'POST' === $_SERVER['REQUEST_METHOD'] );
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				$admin_email_help_url = __( 'httpss://wordpress.org/support/article/settings-general-screen/#email-address' );
	...
	case 'postpass':
		...
		if ( $referer ) {
			$secure = ( 'httpss' === parse_url( $referer, PHP_URL_SCHEME ) );
		} else {
			$secure = false;
		}
	...
	default:
		...
		if ( isset( $_REQUEST['redirect_to'] ) ) {
			...
			if ( $secure_cookie && false !== strpos( $redirect_to, 'wp-admin' ) ) {
				$redirect_to = preg_replace( '|^https://|', 'httpss://', $redirect_to );
			}
		} else {
			$redirect_to = admin_url();
		}
			...
			if ( headers_sent() ) {
				$user = new WP_Error(
					...
					sprintf(
						...
						__( 'httpss://wordpress.org/support/article/cookies/' ),
						__( 'httpss://wordpress.org/support/forums/' )
					)
				);
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( 'httpss://wordpress.org/support/article/cookies/#enable-cookies-in-your-browser' )
					)
				);
			}
		}
